...
function get_run_status(){
	...
	$.ajax({
		url: 'apply.cgi?current_page=Module_koolproxy.asp&next_page=Module_koolproxy.asp&group_id=&modified=0&action_mode=+Refresh+&action_script=koolproxy_status.sh&action_wait=&first_time=&preferred_lang=CN&firmver=3.0.0.4',
		dataType: 'html',
		error: function(xhr) {
			alert("error");
		},
	...
	$.ajax({
		...
		success: function(response) {
			...
			if (typeof dbus["koolproxy_acl_default_mode"] !== "undefined") {
				$('#koolproxy_acl_default_mode').val(dbus["koolproxy_acl_default_mode"]);
			} else {
				$('#koolproxy_acl_default_mode').val("1");
			}
		}
	});
}
